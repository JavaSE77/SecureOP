...
public class deopcmd implements commandHandler {
	...
	public deopcmd (Plugin plugin, CommandSender sender, Command cmd, String[] args) {
		this.plugin = plugin;
		this.sender = sender;
	...
	public deopcmd (Plugin plugin, Player sender, String cmd, String[] args) {
		this.plugin = plugin;
		this.sender = sender;
		this.cmd = cmd;
		this.args = args;
	...
	public void handleCommand() {
		...
		plugin.getLogger().info(sender.getName() + " issued the deop command. This command has been filtered from server logs to prevent passwords being shown in plain text.");
	...
	private void opOnlinePlayer(Player target) {
        ...
        Bukkit.getLogger().info("Player"+ targetName +" has been deoped by " + sender.getName());
	...
	private void opOfflinePlayer() {
        ...
        //inform the logger
        Bukkit.getLogger().info("Offline player " + offlineTarget + " has been deoped by " + sender.getName());
        if (main.plugin.getConfig().getBoolean("BroadcastOP"))
            //if we are supposed to broadcast op, then do so. This will broadcast to everyone. Unless you want this, you
            //should disable it.
          Bukkit.getServer().broadcastMessage(main.plugin.getConfig().getString("DeopMsg").replaceAll("&", "§")
